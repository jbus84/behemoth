...
def run_unanimous_vector_audit(dataset_path):
    ...
    if len(res) > 0:
        print(f"8/8 Vector      | {len(res):<8} | { (res['pnl'] > 0).mean()*100:>8.2f}% | {res['pnl'].mean():>8.3f} bps")

if __name__ == "__main__":
    for y in ["2023", "2024", "2025"]:
        run_unanimous_vector_audit(f"graph_dataset_1m_{y}.parquet")
