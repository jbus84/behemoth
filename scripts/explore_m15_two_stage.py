#!/usr/bin/env python3
"""
M15 two-stage exploration (MOM or REV).
...
Outputs:
- data/analysis/m15_two_stage_holdout_thresholds_<strategy>.csv
- data/analysis/m15_two_stage_wfo_thresholds_<strategy>.csv
"""
...
def _threshold_rows(df: pd.DataFrame, with_year: bool = False) -> pd.DataFrame:
    ...
    for t in EDGE_THRESHOLDS:
        sub = df[(df["edge_score"] > t) & (df["p_up"] >= 0.5)].copy()
...
def main() -> None:
    ...
    holdout_pred = _predict(test, clf, reg_abs, use_features)
    holdout_rows = _threshold_rows(holdout_pred)
    holdout_out = os.path.join(OUT_DIR, f"m15_two_stage_holdout_thresholds_{args.strategy.lower()}.csv")
    ...
    wfo_out = os.path.join(OUT_DIR, f"m15_two_stage_wfo_thresholds_{args.strategy.lower()}.csv")
    wfo_df.to_csv(wfo_out, index=False)

    # Console summary
    view = holdout_rows[
        (holdout_rows["cost_bps"] == 5) & (holdout_rows["edge_threshold"].isin([1, 2, 3, 4, 5]))
    ][
        [
            "edge_threshold",
            "trades",
    ...
    print(f"\nM15 {args.strategy} two-stage holdout (cost=5bps):")
