#!/usr/bin/env python3
"""
Monthly breakdown for MOM-only two-stage model (gross + net).
Uses edge_score from two-stage outputs, but keeps MOM selections only.

Outputs:
- data/analysis/m5_two_stage_monthly_mom_only.csv
- data/analysis/m15_two_stage_monthly_mom_only.csv
"""
...
def main() -> None:
    ...
    out_path = Path(f"data/analysis/{args.bar}_two_stage_monthly_mom_only.csv")
    ...
    pred = pred[pred["chosen_side"] == "MOM"].copy()
