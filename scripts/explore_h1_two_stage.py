#!/usr/bin/env python3
"""
H1 two-stage exploration (MOM or REV).
...
Outputs:
- data/analysis/h1_two_stage_holdout_thresholds_<strategy>.csv
- data/analysis/h1_two_stage_wfo_thresholds_<strategy>.csv
"""
...
def main() -> None:
    ...
    holdout_out = os.path.join(OUT_DIR, f"h1_two_stage_holdout_thresholds_{args.strategy.lower()}.csv")
    ...
    wfo_out = os.path.join(OUT_DIR, f"h1_two_stage_wfo_thresholds_{args.strategy.lower()}.csv")
    wfo_df.to_csv(wfo_out, index=False)

    summary_edges = [0, 0.5, 1, 1.5, 2, 2.5, 3, 4, 5]
    view = holdout_rows[(holdout_rows["cost_bps"] == 5) & (holdout_rows["edge_threshold"].isin(summary_edges))]
    print(f"\nH1 {args.strategy} two-stage holdout (cost=5bps, selected edges):")
