#!/usr/bin/env python3
"""
H4 two-stage exploration (MOM or REV).
...
Outputs:
- data/analysis/h4_two_stage_holdout_thresholds_<strategy>.csv
- data/analysis/h4_two_stage_wfo_thresholds_<strategy>.csv
"""
...
def main() -> None:
    ...
    holdout_out = os.path.join(OUT_DIR, f"h4_two_stage_holdout_thresholds_{args.strategy.lower()}.csv")
    ...
    wfo_out = os.path.join(OUT_DIR, f"h4_two_stage_wfo_thresholds_{args.strategy.lower()}.csv")
    wfo_df.to_csv(wfo_out, index=False)

    view = holdout_rows[(holdout_rows["cost_bps"] == 5) & (holdout_rows["edge_threshold"].isin([4, 5]))]
    print(f"\nH4 {args.strategy} two-stage holdout (cost=5bps):")
