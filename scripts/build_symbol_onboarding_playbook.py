#!/usr/bin/env python3
"""Build the symbol onboarding playbook dynamically.

This script produces a markdown guide in docs/strategy_bible/ that documents
the end-to-end flow for onboarding a new symbol into the OCO pipeline.
It reads the actual EURUSD config templates so the playbook stays in sync
with any future config changes.

Regenerated automatically on every `make docs-contract` run.
"""
...
def main() -> None:
    p = argparse.ArgumentParser(description="Build symbol onboarding playbook")
    p.add_argument("--out-md", default="docs/strategy_bible/symbol_onboarding_playbook.md")
    ...
    eurusd_configs = sorted([f.name for f in config_dir.glob("eurusd*.yaml")]) if config_dir.exists() else []
    ...
    for c in eurusd_configs:
        lines.append(f"- `{c}` → `<sym>_{c[7:]}`")
    ...
    out_md.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"wrote symbol onboarding playbook: {out_md}")
