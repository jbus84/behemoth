#!/usr/bin/env python3
"""
Diagnostics for REV loss-streak guardrail (M15).

Outputs:
- data/analysis/rev_guardrail_overall.csv
- data/analysis/rev_guardrail_monthly.csv
- data/analysis/rev_guardrail_session.csv
- data/analysis/rev_guardrail_symbol.csv
"""
...
DATA_PATH = "data/meta_model/events_m15_8yr_v3_rev.csv"
...
def main() -> None:
    ...
    for label, sub in [("baseline", base), ("loss_streak_3_14d", guard)]:
        ...
        overall.append(row)
    pd.DataFrame(overall).to_csv(os.path.join(OUT_DIR, "rev_guardrail_overall.csv"), index=False)
    ...
    pd.concat(monthly, ignore_index=True).to_csv(
        os.path.join(OUT_DIR, "rev_guardrail_monthly.csv"), index=False
    )
    ...
    pd.concat(session, ignore_index=True).to_csv(
        os.path.join(OUT_DIR, "rev_guardrail_session.csv"), index=False
    )
    ...
    pd.concat(symbol, ignore_index=True).to_csv(
        os.path.join(OUT_DIR, "rev_guardrail_symbol.csv"), index=False
    )

    print("Saved:")
    print("- data/analysis/rev_guardrail_overall.csv")
    print("- data/analysis/rev_guardrail_monthly.csv")
    print("- data/analysis/rev_guardrail_session.csv")
    print("- data/analysis/rev_guardrail_symbol.csv")
