#!/usr/bin/env python3
"""
Meta Model Dataset Generator v3 (M1)
...
DATA_DIR = "data/global_1m"
OUTPUT_DIR = "data/meta_model"
os.makedirs(OUTPUT_DIR, exist_ok=True)
PARTS_DIR = os.path.join(OUTPUT_DIR, "m1_parts")
os.makedirs(PARTS_DIR, exist_ok=True)

# === PAIR UNIVERSE ===
PAIRS = [
    # FX & Commodities
    ("EUR/GBP", "EURUSD_1m.parquet", "GBPUSD_1m.parquet", "close_EURUSD", "close_GBPUSD", 1.6, 1.0),
    ("Gold/Oil", "BCOUSD_1m.parquet", "XAUUSD_1m.parquet", "close_BCOUSD", "close_XAUUSD", 3.0, 3.0), 
    ("Oil/Silver", "BCOUSD_1m.parquet", "XAGUSD_1m.parquet", "close_BCOUSD", "close_XAGUSD", 3.0, 3.0),
    ("AUD/NZD", "NZDUSD_1m.parquet", "AUDUSD_1m.parquet", "close_NZDUSD", "close_AUDUSD", 2.0, 2.0),
    ("CAC/NZD", "NZDUSD_1m.parquet", "FRXEUR_1m.parquet", "close_NZDUSD", "close_FRXEUR", 3.0, 3.0),
    ("Gold/Silver", "XAUUSD_1m.parquet", "XAGUSD_1m.parquet", "close_XAUUSD", "close_XAGUSD", 3.0, 3.0),
    # Global Equities
    ("SPX/DAX", "SPXUSD_1m.parquet", "GRXEUR_1m.parquet", "close_SPXUSD", "close_GRXEUR", 3.0, 2.0),
    ("SPX/CAC", "SPXUSD_1m.parquet", "FRXEUR_1m.parquet", "close_SPXUSD", "close_FRXEUR", 3.0, 2.0),
    ("SPX/FTSE", "SPXUSD_1m.parquet", "UKXGBP_1m.parquet", "close_SPXUSD", "close_UKXGBP", 3.0, 2.0),
    ("SPX/Nikkei", "SPXUSD_1m.parquet", "JPXJPY_1m.parquet", "close_SPXUSD", "close_JPXJPY", 3.0, 2.0),
    ("SPX/HK", "SPXUSD_1m.parquet", "HKXHKD_1m.parquet", "close_SPXUSD", "close_HKXHKD", 4.0, 2.0),
    ("SPX/Dow", "SPXUSD_1m.parquet", "UDXUSD_1m.parquet", "close_SPXUSD", "close_UDXUSD", 2.0, 2.0),
    ("SPX/Nas", "SPXUSD_1m.parquet", "NSXUSD_1m.parquet", "close_SPXUSD", "close_NSXUSD", 2.0, 2.0),
    # Extended FX
    ("AUD/CAD", "AUDUSD_1m.parquet", "USDCAD_1m.parquet", "close_AUDUSD", "close_USDCAD", 2.0, 2.0),
    ("EUR/CHF", "EURUSD_1m.parquet", "USDCHF_1m.parquet", "close_EURUSD", "close_USDCHF", 2.0, 2.0),
    ("EUR/JPY", "EURUSD_1m.parquet", "USDJPY_1m.parquet", "close_EURUSD", "close_USDJPY", 2.0, 1.0),
    ("GBP/JPY", "GBPUSD_1m.parquet", "USDJPY_1m.parquet", "close_GBPUSD", "close_USDJPY", 2.0, 1.0),
    ("CHF/JPY", "USDCHF_1m.parquet", "USDJPY_1m.parquet", "close_USDCHF", "close_USDJPY", 2.0, 1.0),
    ("EUR/AUD", "EURUSD_1m.parquet", "AUDUSD_1m.parquet", "close_EURUSD", "close_AUDUSD", 2.0, 2.0),
    ("GBP/AUD", "GBPUSD_1m.parquet", "AUDUSD_1m.parquet", "close_GBPUSD", "close_AUDUSD", 2.0, 2.0),
    ("GBP/CAD", "GBPUSD_1m.parquet", "USDCAD_1m.parquet", "close_GBPUSD", "close_USDCAD", 2.0, 2.0),
    ("NZD/CAD", "NZDUSD_1m.parquet", "USDCAD_1m.parquet", "close_NZDUSD", "close_USDCAD", 2.0, 2.0),
]
...
def _pair_output_path(pair_name: str, parts_dir: str) -> str:
    safe = pair_name.replace("/", "_").replace(" ", "")
    return os.path.join(parts_dir, f"{safe}_m1.csv")


def _filter_pairs(pairs, requested: list[str] | None, limit: int | None):
    if requested:
        requested_set = {p.strip() for p in requested if p.strip()}
        pairs = [p for p in pairs if p[0] in requested_set]
    if limit is not None:
        pairs = pairs[:limit]
    return pairs


def _combine_parts(parts_dir: str, out_path: str) -> None:
    parts = [os.path.join(parts_dir, f) for f in os.listdir(parts_dir) if f.endswith("_m1.csv")]
    if not parts:
        print("No part files found to combine.")
        return
    print(f"Combining {len(parts)} part files...")
    frames = [pl.read_csv(p) for p in parts]
    df_out = pl.concat(frames, how="vertical")
    df_out.write_csv(out_path)
    print(f"Saved combined dataset to {out_path}")


def build_dataset(pairs, parts_dir: str, skip_existing: bool = True):
    print("--- BUILDING META MODEL DATASET v3 (DUAL STRATEGY) ---")
    
    thresh = 1.5
    stop_level = 3.5
    
    # Phase 1/2: Process pairs incrementally
    print("Processing pairs (incremental)...")

    for name, fx, fy, cx, cy, cost_y, cost_x in pairs:
        out_path = _pair_output_path(name, parts_dir)
        ...
        print(f"  {name}: {len(y)} bars (computing Kalman + z-scores)...")
        betas, errors, ret_betas = compute_kalman_states(y, x)
        z_scores = compute_z_scores(errors)

        print(f"  {name}: generating dual-strategy events...")
        ...
        if pair_events:
            pl.DataFrame(pair_events).write_csv(out_path)
            print(f"  Saved {len(pair_events)} events to {out_path}")
        else:
            print(f"  No events found for {name}.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build M1 meta dataset (incremental)")
    parser.add_argument("--pairs", default="", help="Comma-separated pair names to process")
    parser.add_argument("--limit", type=int, default=None, help="Process only first N pairs")
    parser.add_argument("--parts-dir", default=PARTS_DIR)
    parser.add_argument("--combine", action="store_true", help="Combine part files into final CSV")
    parser.add_argument("--skip-existing", action="store_true", default=True)
    args = parser.parse_args()

    if args.combine:
        out_path = os.path.join(OUTPUT_DIR, "events_m1_8yr_v3_dual.csv")
        _combine_parts(args.parts_dir, out_path)
    else:
        requested = [p for p in args.pairs.split(",") if p.strip()] if args.pairs else []
        selected_pairs = _filter_pairs(PAIRS, requested, args.limit)
        build_dataset(selected_pairs, args.parts_dir, skip_existing=args.skip_existing)
