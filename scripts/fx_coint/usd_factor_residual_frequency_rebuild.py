"""Frequency comparison with PROPER bar building from 1000tick close prices.

Builds 5m/15m/30m/1h bars using close_ts (actual bar close time) and close_bid/close_ask,
then joins across pairs cleanly. Tests if cleaner 5m bars improve results.
"""
...
def build_bars(sym: str, freq: str) -> pl.DataFrame:
    """Build proper time bars from 1000tick close prices using close_ts."""
    df = pl.read_parquet(f"data/tick_bars/{sym}_{TICK}.parquet")
    # Use close_ts (actual bar close) and truncate to freq
    df = df.with_columns(
        pl.col("close_ts").dt.truncate(freq).alias("bar_time"),
        pl.col("close_bid").alias("bid"),
        pl.col("close_ask").alias("ask"),
    )
    # Take last close in each time window
    g = (
        df.sort("close_ts")
        .group_by("bar_time")
        .agg(
            pl.col("bid").last().alias(f"bid_{sym}"),
            pl.col("ask").last().alias(f"ask_{sym}"),
        )
        .sort("bar_time")
    )
    return g
...
def run_freq(freq: str, freq_label: str) -> None:
    print(f"\n{'='*60}")
    print(f"  FREQUENCY: {freq_label} (proper close_ts bars)")
    print(f"{'='*60}")

    # Build bars properly per pair, then join
    frames = [build_bars(s, freq) for s in PAIRS]
    df = frames[0]
    for f in frames[1:]:
    ...
    rets = {}
    for s in syms:
        mid = (df[f"bid_{s}"] + df[f"ask_{s}"]).to_numpy() / 2.0
