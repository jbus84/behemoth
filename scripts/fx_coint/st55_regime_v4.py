"""ST55 REGIME FILTER v4 — AND gate with longer lookback for stability.

Lessons:
  - v1 (AND, 20-bar) had best net but fragile years (2022, 2026 negative).
  - v3 (OR, 50-bar ac1) preserved sample but no accuracy lift.

Hypothesis: the v1 fragility was from NOISY short-term regime estimates.
  - A 50-bar lookback for BOTH skew and ac1 gives a more stable regime read.
  - Fewer false "trending" flags in chop years.

Usage: uv run python scripts/fx_coint/st55_regime_v4.py
"""
# ruff: noqa: E402
...
def _regime_features(oriented_returns, entry_idx, lookback=LOOKBACK):
...
def evaluate_regime_v4(panel, n_folds, seed):
    """AND gate with longer lookback."""
    syms = list(panel)
...
def main():
    ...
    rows, yr = evaluate_regime_v4(panel, N_FOLDS, seed=0)

    print("=" * 90)
    print(f"REGIME FILTER v4: AND GATE + LONG LOOKBACK ({LOOKBACK} bars)")
