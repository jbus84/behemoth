#!/usr/bin/env python3
"""
Meta Model Dataset Generator v3 (10m bars)
...
PAIRS = [
    # FX & Commodities
    ("EUR/GBP", "EURUSD_10m.parquet", "GBPUSD_10m.parquet", "close_EURUSD", "close_GBPUSD", 1.6, 1.0),
    ("Gold/Oil", "BCOUSD_10m.parquet", "XAUUSD_10m.parquet", "close_BCOUSD", "close_XAUUSD", 3.0, 3.0),
    ("Oil/Silver", "BCOUSD_10m.parquet", "XAGUSD_10m.parquet", "close_BCOUSD", "close_XAGUSD", 3.0, 3.0),
    ("AUD/NZD", "NZDUSD_10m.parquet", "AUDUSD_10m.parquet", "close_NZDUSD", "close_AUDUSD", 2.0, 2.0),
    ("CAC/NZD", "NZDUSD_10m.parquet", "FRXEUR_10m.parquet", "close_NZDUSD", "close_FRXEUR", 3.0, 3.0),
    ("Gold/Silver", "XAUUSD_10m.parquet", "XAGUSD_10m.parquet", "close_XAUUSD", "close_XAGUSD", 3.0, 3.0),
    # Global Equities
    ("SPX/DAX", "SPXUSD_10m.parquet", "GRXEUR_10m.parquet", "close_SPXUSD", "close_GRXEUR", 3.0, 2.0),
    ("SPX/CAC", "SPXUSD_10m.parquet", "FRXEUR_10m.parquet", "close_SPXUSD", "close_FRXEUR", 3.0, 2.0),
    ("SPX/FTSE", "SPXUSD_10m.parquet", "UKXGBP_10m.parquet", "close_SPXUSD", "close_UKXGBP", 3.0, 2.0),
    ("SPX/Nikkei", "SPXUSD_10m.parquet", "JPXJPY_10m.parquet", "close_SPXUSD", "close_JPXJPY", 3.0, 2.0),
    ("SPX/HK", "SPXUSD_10m.parquet", "HKXHKD_10m.parquet", "close_SPXUSD", "close_HKXHKD", 4.0, 2.0),
    ("SPX/Dow", "SPXUSD_10m.parquet", "UDXUSD_10m.parquet", "close_SPXUSD", "close_UDXUSD", 2.0, 2.0),
    ("SPX/Nas", "SPXUSD_10m.parquet", "NSXUSD_10m.parquet", "close_SPXUSD", "close_NSXUSD", 2.0, 2.0),
    # Extended FX
    ("AUD/CAD", "AUDUSD_10m.parquet", "USDCAD_10m.parquet", "close_AUDUSD", "close_USDCAD", 2.0, 2.0),
    ("EUR/CHF", "EURUSD_10m.parquet", "USDCHF_10m.parquet", "close_EURUSD", "close_USDCHF", 2.0, 2.0),
    ("EUR/JPY", "EURUSD_10m.parquet", "USDJPY_10m.parquet", "close_EURUSD", "close_USDJPY", 2.0, 1.0),
    ("GBP/JPY", "GBPUSD_10m.parquet", "USDJPY_10m.parquet", "close_GBPUSD", "close_USDJPY", 2.0, 1.0),
    ("CHF/JPY", "USDCHF_10m.parquet", "USDJPY_10m.parquet", "close_USDCHF", "close_USDJPY", 2.0, 1.0),
    ("EUR/AUD", "EURUSD_10m.parquet", "AUDUSD_10m.parquet", "close_EURUSD", "close_AUDUSD", 2.0, 2.0),
    ("GBP/AUD", "GBPUSD_10m.parquet", "AUDUSD_10m.parquet", "close_GBPUSD", "close_AUDUSD", 2.0, 2.0),
    ("GBP/CAD", "GBPUSD_10m.parquet", "USDCAD_10m.parquet", "close_GBPUSD", "close_USDCAD", 2.0, 2.0),
    ("NZD/CAD", "NZDUSD_10m.parquet", "USDCAD_10m.parquet", "close_NZDUSD", "close_USDCAD", 2.0, 2.0),
]
...
def build_dataset():
    print("--- BUILDING META MODEL DATASET v3 (10m, DUAL STRATEGY) ---")
    ...
    if len(all_events) > 0:
        ...
        out_path = os.path.join(OUTPUT_DIR, "events_m10_8yr_v3_dual.csv")
        df_out.write_csv(out_path)
        print(f"Saved to {out_path}")

        df_mom = df_out.filter(pl.col("strategy_type") == "MOM")
        df_rev = df_out.filter(pl.col("strategy_type") == "REV")
        out_mom = os.path.join(OUTPUT_DIR, "events_m10_8yr_v3_mom.csv")
        out_rev = os.path.join(OUTPUT_DIR, "events_m10_8yr_v3_rev.csv")
