...
def _run_api_replay(
    *,
    ...
    cmd = [
        ...
        str(tick_root),
        "--runtime-db",
        str(api_root / "runtime.db"),
        "--events-json",
        str(api_root / "events.json"),
        "--repo-predictions-parquet",
        str(_prediction_path(stage_root, symbol)),
        "--repo-stoplimit-detail-csv",
        str(_stop_limit_detail_path(stage_root, symbol)),
        "--reduced-core-state-schedule-csv",
        str(_reduced_schedule_path(stage_root, symbol)),
        "--start-ts",
        start_ts,
        "--end-ts",
        ...
        "true",
        "--out-summary-csv",
        str(api_root / "execution_summary.csv"),
        "--out-checks-csv",
        str(api_root / "execution_checks.csv"),
        "--out-mismatches-csv",
        str(api_root / "execution_mismatches.csv"),
        "--report-out",
        str(api_root / "execution_report.md"),
        "--local-summary-csv",
        str(local_summary_csv),
        "--local-selected-mismatches-csv",
        str(api_root / "selected_mismatches.csv"),
        "--local-runtime-selected-csv",
        str(runtime_selected_csv),
        "--stage12-summary-csv",
        str(stage12_summary_csv),
        "--stage12-checks-csv",
        str(api_root / "stage12_checks.csv"),
        "--stage12-mismatches-csv",
        str(api_root / "stage12_mismatches.csv"),
        "--stage12-report-out",
        str(api_root / "stage12_report.md"),
        "--fail-on-gate",
        "false",
        "--require-selected-parity",
        "true",
    ]
    ok, msg = _run_cmd(cmd, fail_fast=fail_fast)
    if not ok:
        return {
            ...
            "api_confirmation_status": "failed_pipeline",
            "failure_reason": msg,
        }, pd.DataFrame()
