#!/usr/bin/env python3
"""
Robustness tests for MOM strategy (M15) using guardrailed trades.
Covers:
5) Universe drift
...
TRADES_PATH = os.path.join(OUT_DIR, "m15_mom_best_param_trades_guardrail.csv")
WFO_SUMMARY_PATH = os.path.join(OUT_DIR, "m15_mom_full_wfo_param_summary.csv")
...
def param_stability() -> pd.DataFrame:
    ...
    if df.empty:
        return df
    best = df.iloc[0]
    # distance from best in parameter space
    def dist(row):
        return (
    ...
    # summarize stability by distance buckets
    buckets = []
...
def tail_risk_concentration(trades: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = []
    # remove worst N months by total pnl
    ...
    # remove worst N pairs by total pnl
...
def main() -> None:
    ...
    drift.to_csv(os.path.join(OUT_DIR, "m15_universe_drift.csv"), index=False)

    stability = param_stability()
    stability.to_csv(os.path.join(OUT_DIR, "m15_param_stability.csv"), index=False)

    sessions = session_robustness(trades)
    sessions.to_csv(os.path.join(OUT_DIR, "m15_session_robustness.csv"), index=False)

    tail = tail_risk_concentration(trades)
    tail.to_csv(os.path.join(OUT_DIR, "m15_tail_risk_concentration.csv"), index=False)
