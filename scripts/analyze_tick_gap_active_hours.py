#!/usr/bin/env python3
"""
Tick gap sensitivity restricted to active hours, with gap size cap.
Also splits overlap by entry-only vs full-window.
Outputs:
- data/analysis/m5_tick_gap_active_hours.csv
- data/analysis/m15_tick_gap_active_hours.csv
"""
...
ACTIVE_START = 6   # 06:00 UTC
ACTIVE_END = 21    # 21:00 UTC
MAX_GAP_S = 4 * 3600  # exclude gaps longer than 4 hours
...
def _compute_gaps_for_symbol(symbol: str, gaps: list[int]) -> dict[int, tuple[np.ndarray, np.ndarray]]:
    ...
    for fname in files:
        ...
        for g in gaps:
            idx = np.where((delta > g) & (delta <= MAX_GAP_S))[0]
            if len(idx):
                for i in idx:
...
def _entry_in_gap(entry_ts: int, gap_start: np.ndarray, gap_end: np.ndarray) -> bool:
    if len(gap_start) == 0:
        return False
    i = np.searchsorted(gap_start, entry_ts, side="right") - 1
    if i < 0:
        return False
    return gap_end[i] >= entry_ts
...
def main() -> None:
    ...
    for label, path, module in CONFIGS:
        ...
        hours = pd.to_datetime(df["timestamp"], unit="ns", utc=True).dt.hour
        df = df[(hours >= ACTIVE_START) & (hours < ACTIVE_END)].copy()
        ...
        for row in df.itertuples(index=False):
            ...
            if exit_idx >= len(ts):
                continue
            rows.append({"pair": row.pair, "timestamp": int(row.timestamp), "exit_ts": int(ts[exit_idx]), "pnl_bps": float(row.pnl_bps)})
        ...
        for gap in GAP_SECONDS:
            ...
            for row in base_df.itertuples(index=False):
                ...
                if fx is None:
                    flags_entry.append(False)
                    flags_any.append(False)
                    continue
                ...
                entry_gap = _entry_in_gap(int(row.timestamp), start_x, end_x) or _entry_in_gap(int(row.timestamp), start_y, end_y)
                any_gap = _overlaps_gap(int(row.timestamp), int(row.exit_ts), start_x, end_x) or _overlaps_gap(int(row.timestamp), int(row.exit_ts), start_y, end_y)
                flags_entry.append(entry_gap)
                flags_any.append(any_gap)

            base_df[f"gap_{gap}_entry"] = flags_entry
            base_df[f"gap_{gap}_any"] = flags_any

            for subset, flag_col in [("entry_only", f"gap_{gap}_entry"), ("any_overlap", f"gap_{gap}_any")]:
                gap_df = base_df[base_df[flag_col]].copy()
                nongap_df = base_df[~base_df[flag_col]].copy()

                results.append({
                    "gap_s": gap,
                    "subset": subset,
                    "guardrail": False,
                    "gap_overlap_rate": float(base_df[flag_col].mean()),
                    **_metrics(nongap_df),
                })
                results.append({
                    "gap_s": gap,
                    ...
                    "guardrail": True,
                    "gap_overlap_rate": float(base_df[flag_col].mean()),
                    **_metrics(_apply_guardrail(nongap_df)),
                })

        out = pd.DataFrame(results)
        out.to_csv(os.path.join(OUT_DIR, f"{label}_tick_gap_active_hours.csv"), index=False)
        print(f"Saved: {OUT_DIR}/{label}_tick_gap_active_hours.csv")
