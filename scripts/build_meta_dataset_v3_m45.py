#!/usr/bin/env python3
"""
Meta Model Dataset Generator v3 (45m bars)
...
PAIRS = [
    # FX & Commodities
    ("EUR/GBP", "EURUSD_45m.parquet", "GBPUSD_45m.parquet", "close_EURUSD", "close_GBPUSD", 1.6, 1.0),
    ("Gold/Oil", "BCOUSD_45m.parquet", "XAUUSD_45m.parquet", "close_BCOUSD", "close_XAUUSD", 3.0, 3.0),
    ("Oil/Silver", "BCOUSD_45m.parquet", "XAGUSD_45m.parquet", "close_BCOUSD", "close_XAGUSD", 3.0, 3.0),
    ("AUD/NZD", "NZDUSD_45m.parquet", "AUDUSD_45m.parquet", "close_NZDUSD", "close_AUDUSD", 2.0, 2.0),
    ("CAC/NZD", "NZDUSD_45m.parquet", "FRXEUR_45m.parquet", "close_NZDUSD", "close_FRXEUR", 3.0, 3.0),
    ("Gold/Silver", "XAUUSD_45m.parquet", "XAGUSD_45m.parquet", "close_XAUUSD", "close_XAGUSD", 3.0, 3.0),
    # Global Equities
    ("SPX/DAX", "SPXUSD_45m.parquet", "GRXEUR_45m.parquet", "close_SPXUSD", "close_GRXEUR", 3.0, 2.0),
    ("SPX/CAC", "SPXUSD_45m.parquet", "FRXEUR_45m.parquet", "close_SPXUSD", "close_FRXEUR", 3.0, 2.0),
    ("SPX/FTSE", "SPXUSD_45m.parquet", "UKXGBP_45m.parquet", "close_SPXUSD", "close_UKXGBP", 3.0, 2.0),
    ("SPX/Nikkei", "SPXUSD_45m.parquet", "JPXJPY_45m.parquet", "close_SPXUSD", "close_JPXJPY", 3.0, 2.0),
    ("SPX/HK", "SPXUSD_45m.parquet", "HKXHKD_45m.parquet", "close_SPXUSD", "close_HKXHKD", 4.0, 2.0),
    ("SPX/Dow", "SPXUSD_45m.parquet", "UDXUSD_45m.parquet", "close_SPXUSD", "close_UDXUSD", 2.0, 2.0),
    ("SPX/Nas", "SPXUSD_45m.parquet", "NSXUSD_45m.parquet", "close_SPXUSD", "close_NSXUSD", 2.0, 2.0),
    # Extended FX
    ("AUD/CAD", "AUDUSD_45m.parquet", "USDCAD_45m.parquet", "close_AUDUSD", "close_USDCAD", 2.0, 2.0),
    ("EUR/CHF", "EURUSD_45m.parquet", "USDCHF_45m.parquet", "close_EURUSD", "close_USDCHF", 2.0, 2.0),
    ("EUR/JPY", "EURUSD_45m.parquet", "USDJPY_45m.parquet", "close_EURUSD", "close_USDJPY", 2.0, 1.0),
    ("GBP/JPY", "GBPUSD_45m.parquet", "USDJPY_45m.parquet", "close_GBPUSD", "close_USDJPY", 2.0, 1.0),
    ("CHF/JPY", "USDCHF_45m.parquet", "USDJPY_45m.parquet", "close_USDCHF", "close_USDJPY", 2.0, 1.0),
    ("EUR/AUD", "EURUSD_45m.parquet", "AUDUSD_45m.parquet", "close_EURUSD", "close_AUDUSD", 2.0, 2.0),
    ("GBP/AUD", "GBPUSD_45m.parquet", "AUDUSD_45m.parquet", "close_GBPUSD", "close_AUDUSD", 2.0, 2.0),
    ("GBP/CAD", "GBPUSD_45m.parquet", "USDCAD_45m.parquet", "close_GBPUSD", "close_USDCAD", 2.0, 2.0),
    ("NZD/CAD", "NZDUSD_45m.parquet", "USDCAD_45m.parquet", "close_NZDUSD", "close_USDCAD", 2.0, 2.0),
]
...
def compute_features_at_entry(i, y, x, betas, errors, ret_betas, z_scores, ts):
    ...
    vol_y = np.std(np.diff(y[max(0, i - 100) : i + 1]))
    vol_x = np.std(np.diff(x[max(0, i - 100) : i + 1]))
    features["vol_ratio"] = round(vol_y / vol_x if vol_x > 0 else 1.0, 3)

    if i >= 500:
        corr = np.corrcoef(y[i - 500 : i], x[i - 500 : i])[0, 1]
        features["correlation_500"] = round(corr, 3)
    else:
        features["correlation_500"] = 0.0

    if i >= 500:
        spread = y[i - 500 : i] - betas[i] * x[i - 500 : i]
        slope = np.polyfit(np.arange(len(spread)), spread, 1)[0]
...
def build_dataset():
    print("--- BUILDING META MODEL DATASET v3 (45m, DUAL STRATEGY) ---")
    ...
    for name, state in pair_states.items():
        ...
        betas, errors, ret_betas, z_scores = (
            state["betas"],
            state["errors"],
            state["ret_betas"],
            state["z_scores"],
    ...
    if len(all_events) > 0:
        ...
        out_path = os.path.join(OUTPUT_DIR, "events_m45_8yr_v3_dual.csv")
        df_out.write_csv(out_path)
        print(f"Saved to {out_path}")

        df_mom = df_out.filter(pl.col("strategy_type") == "MOM")
        df_rev = df_out.filter(pl.col("strategy_type") == "REV")
        out_mom = os.path.join(OUTPUT_DIR, "events_m45_8yr_v3_mom.csv")
        out_rev = os.path.join(OUTPUT_DIR, "events_m45_8yr_v3_rev.csv")
        ...
        print(
            f"  Mean: {np.mean(mom_pnl):.2f} | Median: {np.median(mom_pnl):.2f} | P5: {np.percentile(mom_pnl, 5):.2f} | P95: {np.percentile(mom_pnl, 95):.2f}"
        )

        print(f"\nREVERSION: {len(rev_events)} trades")
        print(
            f"  Mean: {np.mean(rev_pnl):.2f} | Median: {np.median(rev_pnl):.2f} | P5: {np.percentile(rev_pnl, 5):.2f} | P95: {np.percentile(rev_pnl, 95):.2f}"
        )
        ...
        for pair in pair_states.keys():
            ...
            if pair_mom and pair_rev:
                print(f"{pair}:")
                print(
                    f"  MOM: n={len(pair_mom)}, mean={np.mean(pair_mom):.2f}, med={np.median(pair_mom):.2f}, p5={np.percentile(pair_mom, 5):.2f}, p95={np.percentile(pair_mom, 95):.2f}"
                )
                print(
                    f"  REV: n={len(pair_rev)}, mean={np.mean(pair_rev):.2f}, med={np.median(pair_rev):.2f}, p5={np.percentile(pair_rev, 5):.2f}, p95={np.percentile(pair_rev, 95):.2f}"
                )
    else:
