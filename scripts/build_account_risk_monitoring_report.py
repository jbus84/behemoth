#!/usr/bin/env python3
"""Build account risk allocator monitoring metrics, alerts, and markdown report."""
    ...
    *,
    ...
    try:
        events = (
            ...
            if _table_exists(con, "account_risk_allocator_events")
            else pd.DataFrame()
        )
        reservations = (
            _safe_query(
    ...
    for sym in symbols:
        ...
        metric_values = {
            "ACCOUNT_RISK_ALLOC_BLOCK_RATE": float(blocked) / denom,
            "ACCOUNT_RISK_ALLOC_BUDGET_EXCEEDED_RATE": float(block_budget) / denom,
            "ACCOUNT_RISK_ALLOC_PIP_VALUE_UNAVAILABLE_RATE": float(block_pip_unavailable) / denom,
            "ACCOUNT_RISK_ALLOC_STALE_PENDING_COUNT": float(stale_pending),
            "ACCOUNT_RISK_ALLOC_OPEN_WITHOUT_BROKER_POS_COUNT": float(open_without_broker),
            "ACCOUNT_RISK_ALLOC_ADMITTED_MISSING_RESERVATION_ID_COUNT": float(
                admitted_missing_reservation_id + admitted_unknown_reservation_id
            ),
            "ACCOUNT_RISK_ALLOC_EVENT_ROWS_LOOKBACK": float(preselected_total),
            "ACCOUNT_RISK_ALLOC_ADMITTED_ROWS_LOOKBACK": float(admitted),
            "ACCOUNT_RISK_ALLOC_BLOCKED_ROWS_LOOKBACK": float(blocked),
            "ACCOUNT_RISK_ALLOC_STALE_OPEN_COUNT": float(stale_open),
        }
        ...
        for spec in METRIC_SPECS:
            ...
            alert_rows.append(
                {
                    "source_alert": "account_risk_allocator",
                    "symbol": sym,
    ...
    lines: list[str] = []
    lines.append("# Account Risk Allocator Monitoring Report")
    lines.append("")
...
def main() -> None:
    p = argparse.ArgumentParser(description="Build account risk allocator monitoring report")
    ...
    p.add_argument(
        "--out-metrics-csv",
        default="data/analysis/tick_opportunity_mining/account_risk_allocator_monitoring_metrics.csv",
    )
    p.add_argument(
        "--out-alerts-csv",
        default="data/analysis/tick_opportunity_mining/account_risk_allocator_monitoring_alerts.csv",
    )
    p.add_argument(
        "--report-out",
