#!/usr/bin/env python3
"""
Diagnostics for MOM/REV loss-streak guardrail on M5 dataset.
...
DATA_PATH = "data/events/events_m5_8yr_v3_dual.csv"
...
def main() -> None:
    ...
    pd.DataFrame(rows_overall).to_csv(os.path.join(OUT_DIR, "m5_guardrail_overall.csv"), index=False)
    pd.concat(rows_month, ignore_index=True).to_csv(os.path.join(OUT_DIR, "m5_guardrail_monthly.csv"), index=False)
    pd.concat(rows_session, ignore_index=True).to_csv(os.path.join(OUT_DIR, "m5_guardrail_session.csv"), index=False)
    pd.concat(rows_symbol, ignore_index=True).to_csv(os.path.join(OUT_DIR, "m5_guardrail_symbol.csv"), index=False)

    print("Saved:")
    print("- data/analysis/m5_guardrail_overall.csv")
    print("- data/analysis/m5_guardrail_monthly.csv")
    print("- data/analysis/m5_guardrail_session.csv")
    print("- data/analysis/m5_guardrail_symbol.csv")
