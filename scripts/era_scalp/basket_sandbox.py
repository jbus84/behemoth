...
try:
    exec(src, ns)
    out = np.asarray(ns[fn](ctx), dtype=float)
    if out.shape != (ctx.n_bars, ctx.n_sym):
        raise ValueError(f"{fn} shape {out.shape} != (n_bars {ctx.n_bars}, n_sym {ctx.n_sym})")
...
def run_program(src: str, ctx: BasketContext, timeout: float = 10.0,
                required_fn: str = "score"):
    """Return (output_array | None, error | None, logs). `required_fn` is the program
    entry-point name the sandbox requires and executes (e.g. 'score' or 'deploy')."""
