#!/usr/bin/env python3
"""
Meta Model Dataset Generator v3 (H2)
...
PAIRS = [
    # FX & Commodities
    ("EUR/GBP", "EURUSD_1h.parquet", "GBPUSD_1h.parquet", "close_EURUSD", "close_GBPUSD", 1.6, 1.0),
    ("Gold/Oil", "BCOUSD_1h.parquet", "XAUUSD_1h.parquet", "close_BCOUSD", "close_XAUUSD", 3.0, 3.0),
    ("Oil/Silver", "BCOUSD_1h.parquet", "XAGUSD_1h.parquet", "close_BCOUSD", "close_XAGUSD", 3.0, 3.0),
    ("AUD/NZD", "NZDUSD_1h.parquet", "AUDUSD_1h.parquet", "close_NZDUSD", "close_AUDUSD", 2.0, 2.0),
    ("CAC/NZD", "NZDUSD_1h.parquet", "FRXEUR_1h.parquet", "close_NZDUSD", "close_FRXEUR", 3.0, 3.0),
    ("Gold/Silver", "XAUUSD_1h.parquet", "XAGUSD_1h.parquet", "close_XAUUSD", "close_XAGUSD", 3.0, 3.0),
    # Global Equities
    ("SPX/DAX", "SPXUSD_1h.parquet", "GRXEUR_1h.parquet", "close_SPXUSD", "close_GRXEUR", 3.0, 2.0),
    ("SPX/CAC", "SPXUSD_1h.parquet", "FRXEUR_1h.parquet", "close_SPXUSD", "close_FRXEUR", 3.0, 2.0),
    ("SPX/FTSE", "SPXUSD_1h.parquet", "UKXGBP_1h.parquet", "close_SPXUSD", "close_UKXGBP", 3.0, 2.0),
    ("SPX/Nikkei", "SPXUSD_1h.parquet", "JPXJPY_1h.parquet", "close_SPXUSD", "close_JPXJPY", 3.0, 2.0),
    ("SPX/HK", "SPXUSD_1h.parquet", "HKXHKD_1h.parquet", "close_SPXUSD", "close_HKXHKD", 4.0, 2.0),
    ("SPX/Dow", "SPXUSD_1h.parquet", "UDXUSD_1h.parquet", "close_SPXUSD", "close_UDXUSD", 2.0, 2.0),
    ("SPX/Nas", "SPXUSD_1h.parquet", "NSXUSD_1h.parquet", "close_SPXUSD", "close_NSXUSD", 2.0, 2.0),
    # Extended FX
    ("AUD/CAD", "AUDUSD_1h.parquet", "USDCAD_1h.parquet", "close_AUDUSD", "close_USDCAD", 2.0, 2.0),
    ("EUR/CHF", "EURUSD_1h.parquet", "USDCHF_1h.parquet", "close_EURUSD", "close_USDCHF", 2.0, 2.0),
    ("EUR/JPY", "EURUSD_1h.parquet", "USDJPY_1h.parquet", "close_EURUSD", "close_USDJPY", 2.0, 1.0),
    ("GBP/JPY", "GBPUSD_1h.parquet", "USDJPY_1h.parquet", "close_GBPUSD", "close_USDJPY", 2.0, 1.0),
    ("CHF/JPY", "USDCHF_1h.parquet", "USDJPY_1h.parquet", "close_USDCHF", "close_USDJPY", 2.0, 1.0),
    ("EUR/AUD", "EURUSD_1h.parquet", "AUDUSD_1h.parquet", "close_EURUSD", "close_AUDUSD", 2.0, 2.0),
    ("GBP/AUD", "GBPUSD_1h.parquet", "AUDUSD_1h.parquet", "close_GBPUSD", "close_AUDUSD", 2.0, 2.0),
    ("GBP/CAD", "GBPUSD_1h.parquet", "USDCAD_1h.parquet", "close_GBPUSD", "close_USDCAD", 2.0, 2.0),
    ("NZD/CAD", "NZDUSD_1h.parquet", "USDCAD_1h.parquet", "close_NZDUSD", "close_USDCAD", 2.0, 2.0),
]
...
def load_pair_data(fx, fy, cx, cy):
    try:
        ...
        df_x = pl.read_parquet(p_x).rename({cx: "X"}).sort("timestamp")
        df_y = pl.read_parquet(p_y).rename({cy: "Y"}).sort("timestamp")

        # Resample 1h -> 2h
        df_x = df_x.group_by_dynamic("timestamp", every="2h").agg([pl.col("X").last()])
        df_y = df_y.group_by_dynamic("timestamp", every="2h").agg([pl.col("Y").last()])
...
def build_dataset():
    print("--- BUILDING META MODEL DATASET v3 (DUAL STRATEGY, H2) ---")
    ...
    if len(all_events) > 0:
        ...
        out_path = os.path.join(OUTPUT_DIR, "events_h2_8yr_v3_dual.csv")
        ...
        out_mom = os.path.join(OUTPUT_DIR, "events_h2_8yr_v3_mom.csv")
        out_rev = os.path.join(OUTPUT_DIR, "events_h2_8yr_v3_rev.csv")
