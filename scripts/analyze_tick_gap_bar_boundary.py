#!/usr/bin/env python3
"""
Tick gap sensitivity restricted to entry/exit bars only.
Uses active hours and excludes gaps > 4h.
Outputs:
- data/analysis/m5_tick_gap_bar_boundary.csv
- data/analysis/m15_tick_gap_bar_boundary.csv
"""
    ...
    for label, path, module in CONFIGS:
        ...
        for gap in GAP_SECONDS:
            ...
            for row in base_df.itertuples(index=False):
                ...
                exit_gap = _entry_in_gap(int(row.exit_ts), start_x, end_x) or _entry_in_gap(int(row.exit_ts), start_y, end_y)
                flags_entry.append(entry_gap)
                flags_exit.append(exit_gap)

            base_df[f"gap_{gap}_entry"] = flags_entry
            base_df[f"gap_{gap}_exit"] = flags_exit

            for subset, flag_col in [("entry_only", f"gap_{gap}_entry"), ("exit_only", f"gap_{gap}_exit")]:
        ...
        out.to_csv(os.path.join(OUT_DIR, f"{label}_tick_gap_bar_boundary.csv"), index=False)
        print(f"Saved: {OUT_DIR}/{label}_tick_gap_bar_boundary.csv")
