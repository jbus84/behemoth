#!/usr/bin/env python3
"""
M5 Meta Model Training and Holdout Reporting
...
MODEL_DIR = "models/meta_model_m5"
...
THRESHOLDS = [0, 5, 8, 10, 12, 15, 18, 20, 25, 30]
...
def main() -> None:
    ...
    model_path = os.path.join(MODEL_DIR, "catboost_m5_reg.cbm")
    ...
    overall_path = os.path.join(ANALYSIS_DIR, "m5_holdout_overall_metrics.csv")
    overall_df.to_csv(overall_path, index=False)

    thresh_overall = pd.DataFrame(_threshold_rows(y_test, y_pred, THRESHOLDS))
    thresh_overall_path = os.path.join(ANALYSIS_DIR, "m5_holdout_thresholds_overall.csv")
    ...
    by_type_metrics_path = os.path.join(ANALYSIS_DIR, "m5_holdout_metrics_by_type.csv")
    ...
    by_type_thresh_path = os.path.join(ANALYSIS_DIR, "m5_holdout_thresholds_by_type.csv")
    by_type_thresh.to_csv(by_type_thresh_path, index=False)

    event_level = _event_level_rows(test_pd, THRESHOLDS)
    event_level_path = os.path.join(ANALYSIS_DIR, "m5_holdout_event_level_thresholds.csv")
