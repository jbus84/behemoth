...
def run(
    *,
    ...
    if df.empty:
        msg = f"No predictions found in {predictions_parquet} matching model_month {month} for {symbol}"
    ...
    def api_logic(row):
        ...
        thr = float(schedule[day_str]) if schedule and day_str in schedule else static_exec
    ...
    if mismatch_count > 0:
        logger.error(
            "Parity Failure: %d mismatches found! (Rate: %.4f)", mismatch_count, mismatch_rate
        )
...
def _write_summary(
    path: Path, symbol: str, success: bool, mismatches: int, extra: int, verdict: str
):
