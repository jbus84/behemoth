#!/usr/bin/env python3
"""
Report M5 holdout threshold stats on trained CatBoost model.
...
MODEL_PATH = "models/meta_model_m5/catboost_m5_reg.cbm"
...
def main() -> None:
    ...
    sample_out = os.path.join(OUT_DIR, "m5_threshold_grid_sample_by_type.csv")
    ...
    event_out = os.path.join(OUT_DIR, "m5_threshold_grid_event_by_type.csv")
    event_metrics.to_csv(event_out, index=False)

    focus = sample_df[
        (sample_df["bucket"] == "ALL")
        & (sample_df["cost_bps"] == 5)
        & (sample_df["pred_threshold"].isin([5, 8, 10, 12, 15]))
    ].sort_values("pred_threshold")
    print("\nM5 sample-level focus (ALL, cost=5bps):")
    print(
        focus[
            [
                "pred_threshold",
