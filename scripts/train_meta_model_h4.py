#!/usr/bin/env python3
"""
H4 Meta Model Two-Stage Training (MOM-only)
...
DATA_PATH = "data/meta_model/events_h4_8yr_v3_dual.csv"
MODEL_DIR = "models/meta_model_h4"
...
def main() -> None:
    ...
    # Save models
    clf_path = os.path.join(MODEL_DIR, "catboost_h4_clf.cbm")
    reg_path = os.path.join(MODEL_DIR, "catboost_h4_reg.cbm")
    ...
    holdout_out = os.path.join(ANALYSIS_DIR, "h4_two_stage_holdout_thresholds_mom_only.csv")
    ...
    wfo_out = os.path.join(ANALYSIS_DIR, "h4_two_stage_wfo_thresholds_mom_only.csv")
    wfo_df.to_csv(wfo_out, index=False)

    view = holdout_rows[(holdout_rows["cost_bps"] == 5) & (holdout_rows["edge_threshold"].isin([4, 5]))]
    print("\nH4 two-stage holdout (cost=5bps, MOM-only):")
