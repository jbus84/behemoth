...
from .cache import cache_get_position, cache_set_position, cache_invalidate_position
from .db import Base, engine, get_db
from .models import Position, Order, PositionEvent, IdempotencyKey, PositionStatus, OrderStatus
from .schemas import (
    PositionCreate,
    PositionOpen,
    PositionClose,
...
from .validation import summary_for_bar
...
def create_position(
    ...
    if idempotency_key:
        db.add(IdempotencyKey(key=idempotency_key, request_hash=req_hash, position_id=pos.id))

    db.add(PositionEvent(position_id=pos.id, event_type="CREATED", payload=_payload_json(payload)))
    db.commit()
    db.refresh(pos)

    cache_set_position(pos.id, PositionResponse.model_validate(pos).model_dump())
    return pos
...
def open_position(position_id: str, payload: PositionOpen, db: Session = Depends(get_db)):
    ...
    if not pos:
        raise HTTPException(status_code=404, detail="Position not found")
    if not can_transition(pos.status, PositionStatus.OPEN):
        raise HTTPException(status_code=409, detail=f"Invalid transition from {pos.status}")
    ...
    db.add(PositionEvent(position_id=pos.id, event_type="OPENED", payload=_payload_json(payload)))
    db.commit()
    db.refresh(pos)
    cache_invalidate_position(pos.id)
    return pos
...
def close_position(position_id: str, payload: PositionClose, db: Session = Depends(get_db)):
    ...
    if not pos:
        raise HTTPException(status_code=404, detail="Position not found")
    if not can_transition(pos.status, PositionStatus.CLOSED):
        raise HTTPException(status_code=409, detail=f"Invalid transition from {pos.status}")
    ...
    db.add(PositionEvent(position_id=pos.id, event_type="CLOSED", payload=_payload_json(payload)))
    db.commit()
    db.refresh(pos)
    cache_invalidate_position(pos.id)
    return pos
...
def cancel_position(position_id: str, db: Session = Depends(get_db)):
    ...
    if not pos:
        raise HTTPException(status_code=404, detail="Position not found")
    if not can_transition(pos.status, PositionStatus.CANCELLED):
        raise HTTPException(status_code=409, detail=f"Invalid transition from {pos.status}")

    pos.status = PositionStatus.CANCELLED
    pos.version += 1
    db.add(PositionEvent(position_id=pos.id, event_type="CANCELLED", payload={"ts": _now().isoformat()}))
    db.commit()
    db.refresh(pos)
    cache_invalidate_position(pos.id)
    return pos
...
def list_orders(position_id: Optional[str] = None, db: Session = Depends(get_db)):
    q = db.query(Order)
    if position_id:
