...
## Deep feature scan (18-concept taxonomy + purged 3-fold cross-fit, budget=24)

After expanding the taxonomy 5→18 (literature-grounded: vol-normalized/intensity-weighted/
acceleration OFI, multi-horizon momentum, jump indicator, vol-cluster/persistence/microprice/
spread/burst regimes, flow×persistence) and wiring the purged K-fold cross-fit, re-ran both
targets. **The conclusion is unchanged — more features and more model rigor did not help.**

| target | V1 | V1pen | V2 | holdout net (best cell) | holdout p₊ | temporal p₊ | robust | dsr_sig |
|--------|----|-------|----|------------------------|-----------|------------|--------|---------|
| forward h12 | -1.858 | -2.078 | -2.458 | -1.263 | 0.443 | 0.01 (14% windows +) | False | False |
| fair h3 | -0.502 | -0.542 | -0.955 | +0.147 | 0.884 | 0.969 (75% windows +) | False | False |

- **forward**: net-negative everywhere; temporal p₊=0.01 means it is *significantly losing*
  across time windows. Clear cost-wall failure.
- **fair**: V1/V2 negative and V2<<V1 (overfit), but the best holdout (q,h) cell shows a small
  +0.147 pips/trade net with p₊=0.884 — the SAME recurring marginal fair-target signal as the
  shallow run (raw 0.144). It is **not durable**: `robust=False` (temporal mu_lo=-0.006 touches
  zero, worst-window p₊=0.17) and `dsr_sig=False`. The robustness/DSR guards exist precisely to
  discount a cherry-picked best-cell mean, and they reject it. +0.147 pips is also ~0.3× the
  EURUSD round-trip cost (~0.49 pips) — i.e. below the live cost floor.

**Decisive read:** with deep, up-to-date literature features + CatBoost + purged cross-fitting,
the binding constraint is confirmed NOT to be model power. The recurring fair-target marginal is
a non-robust artifact, not an edge. (Caveat: `dsr` returned nan for the single-node verdict; the
temporal + p_positive guards independently disqualify, so the verdict stands.)
