...
def run_global_reversion_audit(dataset_path):
    ...
    # Strategy: IF Collective Macro Move > Q99: Fade the move in NSX.
    df_strat = df.with_columns(
        (pl.when(pl.col("macro_mean_120m") > pl.col("macro_tail_thr")).then(pl.col("target_nsx_120m") * 10000 - 1.5) # Macro USD Strong (Short Assets) -> We LONG NSX
          .when(pl.col("macro_mean_120m") < -pl.col("macro_tail_thr")).then(-pl.col("target_nsx_120m") * 10000 - 1.5) # Macro USD Weak (Long Assets) -> We SHORT NSX
          .otherwise(0)).alias("pnl")
    )
    
    res = df_strat.filter(pl.col("pnl") != 0)
    if len(res) > 0:
        print(f"Global Q99 Fade       | {len(res):<8} | { (res['pnl'] > 0).mean()*100:>8.2f}% | {res['pnl'].mean():>8.3f} bps")

if __name__ == "__main__":
    for y in ["2023", "2024", "2025"]:
        run_global_reversion_audit(f"graph_dataset_1m_{y}.parquet")
