...
def test_report_marks_existing_diagnostic_subreports_as_reference_slots() -> None:
    report = render_report(
        manifest={
            "run_id": "unit",
    ...
    assert "Existing Diagnostic Subreport Slots" in report
    assert (
        "- live_audit: `live_audit.md` "
        "(reference path; not generated by this run)"
    ) in report
    assert (
        "- performance_gap: `performance_gap.md` "
        "(reference path; not generated by this run)"
...
def test_run_analysis_writes_manifest_after_referenced_runtime_summary(
    ...
    original_write_df = live_governance_deviation._write_df
    original_write_text = Path.write_text

    def tracking_write_df(path: Path, df: pd.DataFrame) -> None:
        original_write_df(path, df)
        written_files.append(path.name)

    def tracking_write_text(path: Path, data: str, *args: object, **kwargs: object) -> int:
        if path.name == "run_manifest.json":
            assert "window_summary.csv" in written_files
        return original_write_text(path, data, *args, **kwargs)

    monkeypatch.setattr(live_governance_deviation, "_write_df", tracking_write_df)
