...
def test_build_system_reference_docs_writes_all_pages_and_status(tmp_path: Path) -> None:
    ...
    pd.DataFrame(
        [
            ...
            },
            {
                ...
                "final_score": 1.0,
                "is_current_policy": 1,
                "w13_threshold_fragility": 1.2,
            },
        ]
    ).to_csv(tick_root / "oco_threshold_sensitivity.csv", index=False)
    pd.DataFrame(
        [{"symbol": "EURUSD", "scenario_id": "S1_mild", "lb95_per_signal_pips": 0.4}]
    ).to_csv(
        tick_root / "execution_mc_symbol_scenarios.csv",
        index=False,
    )
    pd.DataFrame(
        [{"check_id": "C0", "status": "pass", "severity_if_fail": "low", "metric_value": 0.0}]
    ).to_csv(
    ...
    pd.DataFrame(columns=["symbol", "metric_id", "band"]).to_csv(
        tick_root / "operator_action_status.csv", index=False
    )
    pd.DataFrame(columns=["symbol", "metric_id", "band"]).to_csv(
        tick_root / "oco_alert_disposition.csv", index=False
    )
    ...
    for spec in PAGES:
        ...
        assert "- symbols_covered: `EURUSD,GBPUSD,USDJPY,USDCHF,AUDUSD,USDCAD`" in txt
