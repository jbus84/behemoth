...
def test_build_operator_action_report_accepts_extra_metrics(tmp_path: Path) -> None:
    edge_csv = tmp_path / "edge.csv"
    pd.DataFrame(
        [
            {
...
""",
    ...
    status, _, _ = run(
        edge_metrics_csv=edge_csv,
        extra_metrics_csv=extra_csv,
        rules_yaml=rules_yaml,
        out_status_csv=tmp_path / "status.csv",
        out_report_md=tmp_path / "report.md",
        out_playbook_md=tmp_path / "playbook.md",
        symbols=["EURUSD"],
    )
    row = status.iloc[0]
    assert row["band"] == "red"
