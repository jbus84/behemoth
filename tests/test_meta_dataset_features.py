...
@pytest.mark.parametrize("mod", [m5, m15])
def test_load_pair_data_success(tmp_path, monkeypatch, mod):
    ts = np.array(
    ...
    out = mod.load_pair_data("x.parquet", "y.parquet", "close_X", "close_Y")
...
@pytest.mark.parametrize("mod", [m5, m15])
def test_load_pair_data_missing(tmp_path, monkeypatch, mod):
    monkeypatch.setattr(mod, "DATA_DIR", str(tmp_path))
    out = mod.load_pair_data("missing_x.parquet", "missing_y.parquet", "close_X", "close_Y")
    assert out is None


@pytest.mark.parametrize("mod", [m5, m15])
def test_compute_kalman_states_len1(mod):
    y = np.array([1.0])
    x = np.array([1.0])
...
@pytest.mark.parametrize("mod", [m5, m15])
def test_compute_kalman_states_len2(mod):
    y = np.array([1.0, 1.01])
    x = np.array([1.0, 1.02])
    _, _, ret_betas = mod.compute_kalman_states(y, x)
    assert ret_betas[0] == ret_betas[1]


@pytest.mark.parametrize("mod", [m5, m15])
def test_compute_features_at_entry_small(mod):
    n = 600
    i = 10
...
@pytest.mark.parametrize("mod", [m5, m15])
def test_compute_features_at_entry_large(mod):
    n = 600
    i = 550
