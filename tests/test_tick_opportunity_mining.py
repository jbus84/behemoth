...
def test_tick_opportunity_mining_outputs(tmp_path: Path) -> None:
    ...
    directional, oco, _oco_asym, _no_touch, summary, _fills = run(cfg)
...
def test_run_mines_double_touch(tmp_path: Path) -> None:
    ...
    directional, oco, _oco_asym, _no_touch, _, _fills = run(cfg)
...
def test_run_mines_no_touch(tmp_path: Path) -> None:
    ...
    directional, oco, _oco_asym, no_touch, _, _fills = run(cfg)
...
def test_run_emits_candidate_fills_joinable_to_summary(tmp_path: Path) -> None:
    ...
    directional, oco, _oco_asym, no_touch, summary, fills = run(cfg)
