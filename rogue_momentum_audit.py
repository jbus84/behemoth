...
def run_rogue_momentum_audit(dataset_path):
    ...
    # Strategy: MOMENTUM (Follow the Rogue)
    # If NSX is too strong (Rogue > Thr), it will likely CONTINUE to be strong. LONG NSX.
    # If NSX is too weak (Rogue < -Thr), it will likely CONTINUE to be weak. SHORT NSX.
    ...
    for q in [0.95, 0.99]:
        ...
        df_strat = df.with_columns([
            ((pl.col("nsx_rogue_120m") > pl.col("q_thr"))).alias("sig_long"), # NSX is stronger than macro trend -> Long
            ((pl.col("nsx_rogue_120m") < -pl.col("q_thr"))).alias("sig_short") # NSX is weaker than macro trend -> Short
        ])
        ...
        label = f"Q {int(q*100)} Momentum"
        if len(res) > 0:
            print(f"{label:<25} | {len(res):<8} | { (res['pnl'] > 0).mean()*100:>8.2f}% | {res['pnl'].mean():>8.3f} bps")

if __name__ == "__main__":
    for y in ["2023", "2024", "2025"]:
        run_rogue_momentum_audit(f"graph_dataset_1m_{y}.parquet")
