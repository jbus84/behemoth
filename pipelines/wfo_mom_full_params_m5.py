#!/usr/bin/env python3
"""
Walk-forward optimization (WFO) for full MOM strategy parameters (M5).
...
def _build_pair_states():
    pair_states = []
    for name, fx, fy, cx, cy, _, _ in m5.PAIRS:
        df = m5.load_pair_data(fx, fy, cx, cy)
        if df is None:
            continue
        y = np.log(df["Y"].to_numpy())
        x = np.log(df["X"].to_numpy())
        ts = df["timestamp"].to_numpy()
        betas, errors, _ = m5.compute_kalman_states(y, x)
        z_map = {lb: m5.compute_z_scores(errors, window=lb) for lb in Z_LOOKBACKS}
...
def main() -> None:
    ...
    grid_path = os.path.join(OUT_DIR, "m5_mom_full_wfo_grid.csv")
    grid_df.to_csv(grid_path, index=False)

    best_df = pd.DataFrame(best_rows)
    best_path = os.path.join(OUT_DIR, "m5_mom_full_wfo_best_folds.csv")
    ...
    summary_path = os.path.join(OUT_DIR, "m5_mom_full_wfo_param_summary.csv")
    ...
    if not summary.empty:
        ...
        out_trades.to_csv(os.path.join(OUT_DIR, "m5_mom_best_param_trades_guardrail.csv"), index=False)
