...
def run_balanced_strategy_audit():
    print(">>> BALANCED STRATEGY AUDIT (More Trades, No D10) <<<")
    ...
    # Calculate Deciles (Test Set)
    deciles = np.percentile(vol_test, np.linspace(0, 100, 11))
    
    # Define D7-D9 Range
    d7_low = deciles[6]
    d10_low = deciles[9] # Start of D10
    
    print(f"\nVolatility Filter: {d7_low:.2f} <= Vol < {d10_low:.2f} (Deciles 7, 8, 9)")
    print(f"Excluding D10 (>{d10_low:.2f}) and D1-D6 (<{d7_low:.2f})")
