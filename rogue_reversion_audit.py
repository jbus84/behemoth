...
def run_rogue_reversion_audit(dataset_path):
    if not os.path.exists(dataset_path): return
    df = pl.read_parquet(dataset_path)
    
    print(f"\n>>> 60m ROGUE REVERSION AUDIT FOR {dataset_path} <<<")
    ...
    for div_bps in [15.0, 20.0, 30.0]:
        t = div_bps / 10000
        
        # Strategy: FADE the 60m Rogue Divergence
        # If NSX outperformed SPX significantly -> SHORT NSX
        # If NSX underperformed SPX significantly -> LONG NSX
...
if __name__ == "__main__":
    for y in ["2023", "2024", "2025"]:
        run_rogue_reversion_audit(f"graph_dataset_1m_{y}.parquet")
