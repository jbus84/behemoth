...
def extract_full_year_features(idx_name: str, tick_root: str, horizon_s: int = 120):
    print(
        f"\n>>> EXTRACTING REGIME FEATURES FOR {idx_name} (FULL YEAR 2025, {horizon_s}s HORIZON) <<<"
    )
    ...
    for ym in months:
        ...
        try:
            idx = (
                ...
                .select(["timestamp", "mid", "ask", "bid"])
                .sort("timestamp")
            )
            idx = idx.with_columns(
                ((pl.col("ask") - pl.col("bid")) / pl.col("mid") * 10000).alias("spread")
            )
            idx = idx.with_columns(pl.col("mid").pct_change().alias("idx_ret"))
            ...
            for fx_name in fx_pairs:
                ...
                combined = combined.with_columns(pl.col("fx_px").pct_change().alias("fx_ret"))
                ...
                combined = combined.with_columns(
                    [
                        ...
                        (pl.col("mid").pct_change().rolling_std(window_size=30) * 10000).alias(
                            "idx_vol_30s"
                        ),
                ...
                ).with_columns((pl.col("timestamp") + datetime.timedelta(seconds=horizon_s)).alias("target_time"))

                targets = targets.with_columns(pl.col("target_time").dt.cast_time_unit("ns"))
                ...
                labels = labels.with_columns(
                    [
                        (((pl.col("mid_after") / pl.col("mid")) - 1) * 10000).alias(
                            "fwd_ret_bps"
                        ),
                ...
                labels = labels.with_columns(
                    ((pl.col("fwd_ret_bps") * pl.col("fx_ret_5s").sign()) > 0)
                    .cast(pl.Int8)
                    .alias("target_trend")
                ).drop_nulls()
    ...
    if all_chunks:
        ...
        output_file = f"full_year_dataset_{idx_name}_{horizon_s}s.parquet"
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract full-year regime features with a configurable horizon."
    )
    ...
    parser.add_argument("--horizon", type=int, default=120, help="Forward horizon in seconds")
    args = parser.parse_args()

    extract_full_year_features(args.idx, args.tick_root, args.horizon)
