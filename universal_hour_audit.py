...
def find_universal_hours():
    ...
    for year, path in paths.items():
        ...
        # Eval 7/8 Consensus
        df = df.with_columns(
            (pl.when(pl.col("consensus_up") >= 7).then(pl.col("target_nsx_15m") * 10000 - 1.5)
              .when(pl.col("consensus_down") >= 7).then(-pl.col("target_nsx_15m") * 10000 - 1.5)
              .otherwise(0)).alias("pnl")
        )
        
        stats = df.group_by("hour_utc").agg([
            pl.col("pnl").mean().alias("avg_pnl"),
            pl.col("pnl").count().alias("signals") # Note: this counts all rows. Let's fix.
        ])
    ...
    # Join and Find Universals
    print("\n>>> UNIVERSAL ALPHA HOUR AUDIT (PROFITABLE IN 2023, 2024, 2025) <<<")
    ...
    for h in range(24):
        ...
        if p23 > 0 and p24 > 0 and p25 > 0:
            print(f"{h:<12} | {p23:>8.3f} | {p24:>8.3f} | {p25:>8.3f} (UNIVERSAL)")
        elif p23 > 0 or p24 > 0 or p25 > 0:
            # Show partially positive hours
             print(f"{h:<12} | {p23:>8.3f} | {p24:>8.3f} | {p25:>8.3f}")

if __name__ == "__main__":
    find_universal_hours()
